    ...
    pub fn merkle_prove_layer(
        ...
        let (left, right) = layouter.assign_region(
            ...
            |mut region| {
                ...
                node_cell.copy_advice(|| "digest", &mut region, self.config.advice[0], 0)?;
                region.assign_advice(|| "element", self.config.advice[1], 0, || path_element)?;
        ...
        let digest = hash2_chip.hash(layouter.namespace(|| "hash2"), left, right)?;
        Ok(digest)
    }
